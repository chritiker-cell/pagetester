/**
 * ChordsView — main view for chord practice (Setup + Practice phases)
...
import { generateChordExercise } from '../utils/chordGenerator';
import { MODE_CONFIGS, KEY_GROUPS } from '../data/chordData';
...
export default function ChordsView() {
  ...
  const handleGenerate = useCallback(() => {
    const exercise = generateChordExercise({
      mode: chordsStore.mode,
      keyGroup: chordsStore.keyGroup,
      timeSignature: chordsStore.timeSignature,
      barCount: chordsStore.barCount,
      showFingering: chordsStore.showFingering,
      showChordNames: chordsStore.showChordNames,
    });
    setGeneratedExercise(exercise);
    setExercise(exercise.id);
    setPhase('practice');
  }, [chordsStore, setExercise]);
  ...
  // ========================
  if (phase === 'setup') {
    const modeCfg = MODE_CONFIGS[chordsStore.mode];
    ...
    return (
      <div className="flex items-start justify-center min-h-[60vh] p-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full max-w-4xl">
          <ChordSelector onGenerate={handleGenerate} />
          ...
          <div className="lg:sticky lg:top-4 space-y-4">
            <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-lg p-6">
              ...
              <p className="text-sm text-neutral-600 dark:text-neutral-300 mb-2">{modeCfg.description}</p>
              <div className="mb-3"><Badge variant="primary" size="sm">{modeCfg.difficulty}</Badge></div>
              <ul className="space-y-1.5 text-sm text-neutral-600 dark:text-neutral-300">
                <li className="flex items-start gap-2">
                  <span className="text-primary-400">•</span>
                  <span>Tonarten: {KEY_GROUPS[chordsStore.keyGroup].label}</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary-400">•</span>
                  <span>Taktart: {chordsStore.timeSignature === 'random' ? 'Zufall' : chordsStore.timeSignature}</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary-400">•</span>
                  <span>Takte: {chordsStore.barCount}</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary-400">•</span>
                  <span>Stimmen: {modeCfg.voiceCount}</span>
                </li>
              </ul>
            </div>
            ...
            <div className="bg-primary-50/50 dark:bg-primary-900/10 rounded-lg p-4 border border-primary-100 dark:border-neutral-700">
              ...
              <p className="text-sm text-neutral-600 dark:text-neutral-300">
                Die Akkorde werden als Klaviersystem (Grand Staff) angezeigt.
                Tempo wird im Practice-Toolbar eingestellt.
              </p>
            </div>
          </div>
        </div>
        ...
        {currentExercise && (
          <div className="flex-1 min-h-0 overflow-auto bg-white">
            <MusicSheet
              ...
              fullscreen
              showChordSymbols={chordsStore.showChordNames}
            />
          </div>
        )}
      </div>
